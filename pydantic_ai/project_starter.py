...
"""
        ...
        if pd.to_datetime(availability_date) <= pd.to_datetime(requested_date):
            ...
            return response_to_customer(
                decision = f"Your order {id} with requested delivery date {requested_date} is confirmed.",
        ...
        return response_to_customer(
                decision = f"Unfortunately your order {id} with requested delivery date {requested_date} has not been validated.",
                status = "ORDER NOT CONFIRMED",
